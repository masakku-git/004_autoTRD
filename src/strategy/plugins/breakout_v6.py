...
v5.0からの変更点:
  - RSI決済をTP1到達後のみに限定: TP1未到達時はRSI判定を行わない
    → TP1到達前にRSI<50で早期決済されるのを防止
  - RSIエグジット閾値: RSI < 50 で残りポジションを決済
    → トレンド回帰の勢いが失われたタイミングで利確
  - max_hold_days: 15 → 60（スイングトレードとして十分な保有期間を確保）
  - トレーリングストップの動的化: ADX強度に応じてトレール幅を調整
    → ADX>30（強トレンド）: 1.5×ATR（利益確保を優先）
    → ADX 20-30（通常トレンド）: 2.0×ATR（利益を伸ばす余地を確保）
    → ADX<20（弱トレンド）: 2.5×ATR（ノイズで退場しないように余裕を持たせる）
  - ブレークイーブンストップはv5を継承
...
class BreakoutV6(BaseStrategy):
    ...
    def check_exit(
        ...
        3. RSI決済（TP1到達後のみ）: RSI < 50 でトレンド勢い喪失と判断し決済
        """
        ...
        # RSI決済: TP1到達後のみ適用
        # TP1到達済み = 段階利確で半分決済済みを意味する
        tp1 = trade_info.get("take_profit_1", 0)
        if tp1 > 0 and highest_price >= tp1:
            rsi = self._calculate_rsi(df["Close"])
            if not pd.isna(rsi) and rsi < self.rsi_exit_threshold:
                return ExitDecision(
                    should_exit=True,
                    reason=(
                        f"RSI exit after TP1: RSI={rsi:.1f} < {self.rsi_exit_threshold} "
                        f"(TP1={tp1:.2f} reached, high={highest_price:.2f})"
                    ),
                )
