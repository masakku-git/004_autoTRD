...
エントリー条件（全て必須）:
  ...
  2. close <= SMA20 × 1.02（SMA20付近まで押し目）
  3. RSI(14)が40〜55（過熱なし、売られすぎでもない「休憩」状態）
  4. 当日出来高 < 20日平均出来高（売り圧力が弱い＝健全な調整）
  5. sp500_trend != "bear"

ストップロスはSMA50サポートまたはATRベース（きつい方を採用）。
トレーリングストップで含み益を保護。
"""
...
class PullbackV1(BaseStrategy):
    ...
    def generate_signals(
        ...
        # 条件2: SMA20付近まで押し目（SMA20の2%以内）
        if current_price > sma20_val * 1.02:
            return None

        # 条件3: RSIが40〜55（「休憩」ゾーン）
        rsi = self._calculate_rsi(close)
        if pd.isna(rsi) or not (self.rsi_low <= rsi <= self.rsi_high):
            return None

        # 条件4: 出来高が平均以下（売り圧力が弱い＝健全な調整）
        avg_volume = float(volume.iloc[-20:].mean())
        if avg_volume <= 0 or current_volume >= avg_volume:
            return None
