...
v1.0からの変更点:
  - RSI条件緩和: 40〜55 → 35〜60（V1では条件が厳しすぎてシグナル数が不足）
  - 出来高条件緩和: < 平均 → < 平均の1.2倍
    → V1の「平均以下」は厳しすぎ。1.2倍以下なら売り圧力は依然として弱い
  - SMA20近接条件緩和: 2%以内 → 3%以内
    → V1では押し目の検出が遅れるケースがあった
  - SL/TP調整: TP2を3.5→4.0×ATR（V1の利確が近すぎて利大損小が不十分だった）
  - 11月の大損失(-$393)対策: 信頼度上限を0.85→0.80に引き下げ、Criticによる
    フィルタリングを強化
